...
class EquidistantDistortion : public DistortionModel
{
 public:
  ...
  void undistort(Eigen::Vector2d &y) const
  {
    ...
    outFile.open("cmp-result.txt",std::ios::app);
    outFile << y[0]<<"\t"<<y[1]<<"\t"<<thetad<<"\t";

    int interation_num = 30;
    theta = thetad; // initial guess
    double theta_d = thetad;
    if (theta_d > 1e-8)
    {
      ...
      for (int j = 0; j < interation_num; j++)
      {
        ...
        theta = theta_d - (_distCoeffs[0] * theta3 + _distCoeffs[1] * theta5 + _distCoeffs[2] * theta7 + _distCoeffs[3] * theta9);
        tmp = j;
    ...
    if (theta_d > 1e-8)
    {
      ...
      for (int j = 0; j < interation_num; j++)
      {
        ...
        if (std::fabs(yk - xk) < 1e-10)
        {
         ...
         theta2 = theta * theta, theta3 = theta2 * theta, theta5 = theta3 * theta2, theta7 = theta5 * theta2, theta9 = theta7 * theta2;
        theta = theta_d - (_distCoeffs[0] * theta3 + _distCoeffs[1] * theta5 + _distCoeffs[2] * theta7 + _distCoeffs[3] * theta9);
        double zk = theta;
      ...
      outFile << tmp << "\t" << theta << "\t"<<theta_early-theta<<"\n";
    }
    outFile.close();
    scaling = tan(theta) / thetad;
    y[0] *= scaling;
