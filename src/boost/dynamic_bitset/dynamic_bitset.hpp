...
public:
    ...
    template <typename B, typename A, typename Type>
    friend void to_array_range(const dynamic_bitset<B, A>& b,
                               Type* result, std::size_t len);

    template <typename B, typename A, typename Type>
    friend void from_array_range(const Type* a, std::size_t len,
...
template <typename Block, typename Allocator, typename Type>
void
to_array_range(const dynamic_bitset<Block, Allocator>& b,
                Type* result, std::size_t len);

template <typename Block, typename Allocator, typename Type>
void
from_array_range(const Type* a, std::size_t len,
                 dynamic_bitset<Block, Allocator>& b);
...
// Provide a simpler and more effective way to read bits
// in dynamic_bitset object.
template <typename Block, typename Allocator, typename Type>
inline void
to_array_range(const dynamic_bitset<Block, Allocator>& b,
                Type *result, std::size_t len)
{
  assert(result != reinterpret_cast<Type*>(0));
  assert((dynamic_bitset<Block, Allocator>::bits_per_block) == 64);
...
// Provide a simpler and more effective way to write bits
// to dynamic_bitset object.
template <typename Block, typename Allocator, typename Type>
inline void
from_array_range(const Type* a, std::size_t len,
                 dynamic_bitset<Block, Allocator>& b)
{
  assert(a != reinterpret_cast<Type*>(0));
  assert((dynamic_bitset<Block, Allocator>::bits_per_block) == 64);
