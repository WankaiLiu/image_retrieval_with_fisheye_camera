...
#include <utility/android_utility.h>
...
#ifdef ANDROID
  inline double distance(const TDescriptor &a, const TDescriptor &b) const;
#endif
...
private:
  ...
  /// provide protection for vector writing
  mutable std::mutex mVWrite;
};
...
TemplatedVocabulary<TDescriptor,F>::TemplatedVocabulary(
  ...
  : m_scoring_object(NULL)
{
  LOGD("loop start load vocabulary\n");
  *this = voc;
}
  ...
  const std::vector<TDescriptor>& features, BowVector &v) const
{
  ...
  {
    int f_size = features.size();
    vector<pair<WordId, WordValue>> words(f_size);

#ifdef LOOPC_USE_OPENMP
#pragma omp parallel for num_threads(2) schedule(dynamic)
    ...
    task_arena.execute([&]{
      tbb::parallel_for(0, f_size, [&](int i) {
#else
    for(int i = 0; i < f_size; i++) {
      ...
      //if(w > 0) v.addWeight(id, w); // not stopped

      // Move the part above out of loop
      // We have to add lock to protect writing of v and fv
      // if enabling multi-threads parallel. This lock would
      // slow down the process. Here we only save the values
      // into vector sequentially for later use
      words[i] = make_pair(id, w);
    }
#ifdef LOOPC_USE_TBB
    );
    });
    ...
    // Add weight and feature in single thread
    for(int i = 0; i < f_size; i++) {
      if(words[i].second > 0) // if (w > 0)
        v.addWeight(words[i].first, words[i].second);
    }
    words.clear(); // release vector memory
    words.shrink_to_fit();
...
#pragma omp parallel for num_threads(2) schedule(dynamic)
    ...
    for(int i = 0; i < f_size; i++) {
      ...
      //if(w > 0) // not stopped
      //{
			//  unique_lock<mutex> lock(mVWrite);
      //  v.addWeight(id, w);
      //  fv.addFeature(nid, i);
      //}
...
#ifdef ANDROID
template<class TDescriptor, class F>
inline double TemplatedVocabulary<TDescriptor,F>::distance
  (const TDescriptor &a, const TDescriptor &b) const
{
  std::size_t num = 0;

  // Here we assume size of arrays is 4, because of 256 bits.
  // Modify it if necessary
  uint64_t va[4] = {0x0};
  uint64_t vb[4] = {0x0};
  int num_blocks = a.num_blocks();
  assert(num_blocks == 4);
  to_array_range(a, va, num_blocks);
  to_array_range(b, vb, num_blocks);

  const uint8_t* pa = reinterpret_cast<const uint8_t*>(&va[0]);
  const uint8_t* pb = reinterpret_cast<const uint8_t*>(&vb[0]);
  uint8x16_t src_a = vld1q_u8(pa);
  uint8x16_t src_b = vld1q_u8(pb);
  uint8x16_t dst = vcntq_u8(veorq_u8(src_a, src_b));
  num += vgetq_lane_u8(dst,  0);
  num += vgetq_lane_u8(dst,  1);
  num += vgetq_lane_u8(dst,  2);
  num += vgetq_lane_u8(dst,  3);
  num += vgetq_lane_u8(dst,  4);
  num += vgetq_lane_u8(dst,  5);
  num += vgetq_lane_u8(dst,  6);
  num += vgetq_lane_u8(dst,  7);
  num += vgetq_lane_u8(dst,  8);
  num += vgetq_lane_u8(dst,  9);
  num += vgetq_lane_u8(dst, 10);
  num += vgetq_lane_u8(dst, 11);
  num += vgetq_lane_u8(dst, 12);
  num += vgetq_lane_u8(dst, 13);
  num += vgetq_lane_u8(dst, 14);
  num += vgetq_lane_u8(dst, 15);

  src_a = vld1q_u8(pa + 16);
  src_b = vld1q_u8(pb + 16);
  dst = vcntq_u8(veorq_u8(src_a, src_b));
  num += vgetq_lane_u8(dst,  0);
  num += vgetq_lane_u8(dst,  1);
  num += vgetq_lane_u8(dst,  2);
  num += vgetq_lane_u8(dst,  3);
...
/*#ifndef WINDOWS
    ...
    m_nodes[nid].descriptor = boost::dynamic_bitset<>(voc.nodes[i].descriptor,voc.nodes[i].descriptor + 4); //modified for debug 32 bit system compatibility issue(boost::dynamic_bitset size()!= rhs.size() issue)

#elif (__SIZEOF_PTRDIFF_T__ == 4)
    m_nodes[nid].descriptor = boost::dynamic_bitset<>(voc.nodes[i].descriptor,voc.nodes[i].descriptor + 8); //modified for debug 32 bit system compatibility issue(boost::dynamic_bitset size()!= rhs.size() issue)
#endif
#endif//WINDOWS*/
    m_nodes[nid].descriptor = boost::dynamic_bitset<uint64_t>(voc.nodes[i].descriptor,voc.nodes[i].descriptor + 256/8/sizeof(uint64_t));
