    ...
    bool treat_image) const
{
  ...
  if(treat_image)
  {
    ...
    // Avoid to repeat gaussian blur filter
    // for the same image
    static uchar* prev_image_data = NULL;
    static cv::Mat prev_im;
    if(aux.data != prev_image_data) {
      prev_image_data = aux.data;
      cv::GaussianBlur(aux, prev_im, ksize, sigma, sigma);
    }
